...
use postgres::types::{BOOL, DATE, FLOAT4, FLOAT8, INT4, INT8, TEXT, TIMESTAMP, VARCHAR};
use serde_json;
    ...
    fn parse_data(row: &Row) -> HashMap<String, Option<Value>> {
        ...
        for column in row.columns().iter() {
            ...
            let value = match column.type_() {
                ...
                &TEXT | &VARCHAR => match row.get::<_, Option<String>>(idx) {
                    Some(d) => Some(Value::STR(d)),
                    None => None,
                },
                &FLOAT8 => match row.get::<_, Option<f64>>(idx) {
                    Some(val) => Some(Value::F64(val)),
                    None => None,
                },
                &FLOAT4 => match row.get::<_, Option<f32>>(idx) {
                    Some(val) => Some(Value::F32(val)),
                    None => None,
                },
                &BOOL => match row.get::<_, Option<bool>>(idx) {
                    Some(_val) => Some(Value::Bool(row.get::<_, bool>(idx))),
